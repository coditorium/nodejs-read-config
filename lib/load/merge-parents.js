...
module.exports.async = function(configPath, opts, callback) {
    opts = opts || {};
    resolvePath.async(configPath, [opts.cwd], (configResolvedPath) => {
...
module.exports.sync = function(configPath, opts) {
    opts = opts || {};
    const configResolvedPath = resolvePath.sync(configPath, [opts.cwd]);
    if (!configResolvedPath) {
...
function isOptional(configPath, opts) {
    ...
    return optional.some((optional) => {
        if (optional instanceof RegExp) {
            return optional.test(configPath);
        }
...
function generateBasedirs(configPath, opts) {
    return [path.dirname(configPath), process.env.PWD, opts && opts.cwd];
}
